...
add_action( 'webfinger_user_data', array( 'SalmonPlugin', 'addUserMagicSig' ), 10, 3 );
add_action( 'webfinger_user_data', array( 'SalmonPlugin', 'addUserSalmon' ), 10, 3 );
...
class SalmonPlugin {
	 ...
	 */
	public static function addXrdDiscovery( $user = null ) {
	 ...
	 */
	public static function addXrdMagicSig( $user ) {
	 ...
	 * @return array
	 */
	public static function addUserMagicSig( $array, $resource, $user ) {
		$sig     = MagicSig::get_public_sig( $user->ID );
		$encoded = MagicSig::to_string( $sig );
	 ...
	 */
	public static function addSalmonLink() {
	 ...
	 * @return array
	 */
	public static function addUserSalmon( $array, $resource, $user ) {
		$salmon           = self::generateApiUrl( $user );
		$array['links'][] = array(
			'rel'  => 'salmon',
			'href' => $salmon,
		);
	 ...
	 * @return string
	 */
	public static function generateApiUrl( $user = null ) {
		if ( $user ) {
	...
	public static function printAtomFeedHeader() {
		SalmonPlugin::printFeedHeader( 'atom' );
	}

	public static function printRssFeedHeader() {
	 ...
	 */
	public static function printFeedHeader( $type ) {
		if ( is_author() ) {
	 ...
	 */
	public static function addSalmonAvatar( $avatar, $id_or_email, $size, $default, $alt = '' ) {
	 ...
	 * @param int $commentId
	 */
	public static function addCrosspostingExtension( $commentId ) {
		// get comment
	 ...
	 */
	public static function addRewriteRules( $wp_rewrite ) {
		global $wp_rewrite;
		$new_rules         = array(
	 ...
	 */
	public static function parseSalmonPost() {
		$user = null;
		// get user by url
		if ( get_query_var( 'author_name' ) ) :
			$user = get_user_by( 'slug', get_query_var( 'author_name' ) );
