...
class SalmonEntry {
	 ...
	 *     of $parent.
	 */
	public function parent_is( $atom, $parent, $breadcrumbs ) {
	 ...
	 */
	public function from_atom( $atom_string ) {
		...
		for ( $i = 0; $atom = $xml_values[ $i ]; $i++ ) {
			...
			switch ( strtolower( $atom['tag'] ) ) {
				case 'id':
					...
					break;
				case 'link':
					if ( 'alternate' == $atom['attributes']['rel'] && SalmonEntry::parent_is( $atom, 'entry', $breadcrumbs ) ) {
			...
			if ( 'link' == $atom['tag'] && 'avatar' == $atom['attributes']['rel'] ) {
